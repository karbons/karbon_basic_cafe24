...
check_requirements() {
    ...
    if [ ! -d "$PROJECT_ROOT/frontend/app" ]; then
        echo -e "${RED}✗ 프론트엔드 폴더가 없습니다: $PROJECT_ROOT/frontend/app${NC}"
        return 1
    fi
...
build_frontend() {
    echo -e "${YELLOW}========================================${NC}"
    echo -e "${YELLOW}  App 빌드${NC}"
    echo -e "${YELLOW}========================================${NC}"
    echo ""
    
    local app_dir="$PROJECT_ROOT/frontend/app"
...
upload_build() {
    ...
    local build_dir="$PROJECT_ROOT/frontend/app/build"
    local remote_path="$SFTP_PATH/app"
    ...
    echo -e "${BLUE}→ /frontend/app/build 파일 업로드 중...${NC}"
...
main() {
    ...
    echo -e "  1. /frontend/app SvelteKit 빌드"
    echo -e "  2. 빌드 파일을 /app 폴더에 업로드"
    echo ""
